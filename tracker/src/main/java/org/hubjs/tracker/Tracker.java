...
import org.hubjs.tracker.dispatcher.DispatchMode;
import org.hubjs.tracker.dispatcher.Dispatcher;
import org.hubjs.tracker.dispatcher.Packet;
import org.hubjs.tracker.tools.Objects;
...
public class Tracker {
    private static final String TAG = Hubjs.tag(Tracker.class);

    // Hubjs default parameter values
    ...
    protected Tracker(Hubjs hubjs, TrackerBuilder config) {
        mHubjs = hubjs;
        ...
        new LegacySettingsPorter(mHubjs).port(this);

        mOptOut = getPreferences().getBoolean(PREF_KEY_TRACKER_OPTOUT, false);

        mDispatcher = mHubjs.getDispatcherFactory().build(this);
        ...
        int[] res = mHubjs.getDeviceHelper().getResolution();
        if (res != null) resolution = String.format("%sx%s", res[0], res[1]);
        mDefaultTrackMe.set(QueryParams.SCREEN_RESOLUTION, resolution);

        mDefaultTrackMe.set(QueryParams.USER_AGENT, mHubjs.getDeviceHelper().getUserAgent());
        mDefaultTrackMe.set(QueryParams.LANGUAGE, mHubjs.getDeviceHelper().getUserLanguage());
    ...
    /**
     * @return true if Hubjs is currently disabled
     */
    ...
    /**
     * Hubjs will use the content of this object to fill in missing values before any transmission.
     ...
     * The Hubjs backend accepts backdated events for up to 24 hours by default.
     * <p>
     * &gt;0 = limit in ms<br>
     * 0 = unlimited<br>
     ...
     * fetch this user ID from your system, and pass it to Hubjs.
     * <p>
    ...
    /**
     * @return a user-id string, either the one you set or the one Hubjs generated for you.
     */
     ...
     * If this value is not set Hubjs will still track visits, but the unique visitors metric might be less accurate.
     */
    ...
    public SharedPreferences getPreferences() {
        if (mPreferences == null) mPreferences = mHubjs.getTrackerPreferences(this);
        return mPreferences;
    }
