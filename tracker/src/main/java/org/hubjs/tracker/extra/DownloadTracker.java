...
public class DownloadTracker {
    protected static final String TAG = Hubjs.tag(DownloadTracker.class);
    ...
    public DownloadTracker(Tracker tracker) {
        this(tracker, getOurPackageInfo(tracker.getHubjs().getContext()));
    }
    ...
    public DownloadTracker(Tracker tracker, @NonNull PackageInfo packageInfo) {
        mTracker = tracker;
        mContext = tracker.getHubjs().getContext();
        mPreferences = tracker.getPreferences();
        mPackMan = tracker.getHubjs().getContext().getPackageManager();
    ...
    private void trackNewAppDownloadInternal(TrackMe baseTrackMe, @NonNull Extra extra) {
        ...
        if (referringApp != null && referringApp.equals(INSTALL_SOURCE_GOOGLE_PLAY)) {
            ...
            String referrerExtras = mTracker.getHubjs().getPreferences().getString(InstallReferrerReceiver.PREF_KEY_INSTALL_REFERRER_EXTRAS, null);
